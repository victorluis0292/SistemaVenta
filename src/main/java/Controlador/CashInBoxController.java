...
private void registrarSaldoInicial() {
    try {
        ...
        int idEmpresa = loginUsuario.getIdEmpresa(); // <-- asegúrate que loginUsuario tenga este getter

        // Usar el nuevo constructor que incluye idEmpresa
    CashInBox cash = new CashInBox(amount, loginUsuario.getNombre(), idEmpresa);


       boolean registrado = dao.registrarSaldoInicial(cash, idEmpresa);
