...
public class MovimientoCajaDao {
    ...
    public boolean registrarMovimiento(MovimientoCaja movimiento) {
        String sql = "INSERT INTO movimientos_caja "
                   + "(tipo, concepto, nota, monto, gasto, fecha, id_turno, id_empresa) "
                   + "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (Connection conn = conexion.getConnection();
            ...
            ps.setInt(7, movimiento.getIdTurnoActual());
            ps.setInt(8, movimiento.getIdEmpresa()); // <-- Guardar empresa activa
